...
<div class="chart-container"><h3>📈 Rolling LTM Trend</h3><div class="chart-wrapper"><canvas id="ltmChart"></canvas></div></div>
<div class="chart-container"><h3>📊 Monthly Sales</h3><div class="chart-wrapper"><canvas id="monthlyChart"></canvas></div></div>
</div>

<div class="chart-section"><h2>📅 Year-over-Year Comparison by Month</h2><div class="chart-wrapper"><canvas id="yoyChart"></canvas></div></div>
...
if mode == "Order Book":
            ...
            try:
                ...
                if ws is None: 
                    ...
                    for r in range(2, ws.max_row + 1):
                        ...
                        dd = ws.cell(r, 5).value  # Delivery date
                        orders.append({
                            'ordernr': ws.cell(r, 1).value, 
                            'orderdatum': od if isinstance(od, datetime) else None,
                            'kundnamn': ws.cell(r, 3).value,
                            'artikelnr': ws.cell(r, 4).value,
                            'leveransdatum': dd if isinstance(dd, datetime) else None,
                            'original_amount': amt,
    ...
    if 'ob_res' in st.session_state:
        ...
        with c4: st.metric("Avg Order", f"{sum(o['belopp_sek'] for o in orders)/len(orders):,.0f}" if orders else "0")
        
        # Create downloadable Excel
        output = BytesIO()
        wb_out = openpyxl.Workbook()
        ws_out = wb_out.active
        ws_out.title = "Cleaned Orders"
        ws_out.append(['Ordernr', 'Orderdatum', 'Kundnamn', 'Artikelnr', 'Leveransdatum', 'Original Amount', 'Currency', 'Belopp SEK'])
        for o in orders:
            ws_out.append([
                o['ordernr'], 
                o['orderdatum'].strftime('%Y-%m-%d') if o['orderdatum'] else '',
                o['kundnamn'],
                o['artikelnr'],
                o['leveransdatum'].strftime('%Y-%m-%d') if o['leveransdatum'] else '',
                o['original_amount'],
                o['original_currency'],
                o['belopp_sek']
            ])
        wb_out.save(output)
        st.download_button(
            label="📥 Download Cleaned Order Book",
            data=output.getvalue(),
            file_name=f"OrderBook_Cleaned_{datetime.now().strftime('%Y%m%d')}.xlsx",
    ...
    _section_label("Files")
    f = st.file_uploader("This month raw data (from Briox)", type=['xlsx'], key="sales")
    ...
    if 'sales_res' in st.session_state:
        ...
        c1, c2, c3, c4 = st.columns(4)
        with c1: st.metric("Article Total", f"{total_art:,.0f} SEK")
        with c2: st.metric("Articles", len(arts))
        with c3: st.metric("Customer Total", f"{total_cust:,.0f} SEK")
        with c4: st.metric("Customers", len(custs))
        
        # Create downloadable Excel with both sheets
        output = BytesIO()
        wb_out = openpyxl.Workbook()
        
        # Articles sheet
        ws_art = wb_out.active
        ws_art.title = "Articles"
        ws_art.append(['Artikelnr', 'Artikelnamn', 'Summa'])
        for a in arts:
            ws_art.append([a['artikelnr'], a['artikelnamn'], a['summa']])
        
        # Customers sheet
        ws_cust = wb_out.create_sheet("Customers")
        ws_cust.append(['Kundnr', 'Kund', 'Summa'])
        for c in custs:
        ...
        st.download_button(
            ...
            data=output.getvalue(),
            file_name=f"Sales_Cleaned_{datetime.now().strftime('%Y%m%d')}.xlsx",
            mime="application/vnd.openxmlformats-officedocument.spreadsheetml.sheet",
            use_container_width=True
        )
        
        st.markdown('<p style="color:#6B7280;font-size:12px;margin-top:8px;">This file contains Article and Customer sheets ready to paste into Sales_work_file.xlsx</p>', unsafe_allow_html=True)
