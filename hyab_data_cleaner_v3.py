...
if mode == "Order Book":
    ...
    if 'sales_res' in st.session_state:
        ...
        ws_t20a.append([])
        ws_t20a.append(['#', 'Article No', 'Article Name', 'Amount excl. VAT', '% of Total'])
        for i, a in enumerate(arts[:20], 1):
            ws_t20a.append([i, a['artikelnr'], a['artikelnamn'], a['summa'], a['summa']/total_art*100 if total_art > 0 else 0])
        for col in ['D', 'E']:
            for row in range(4, 24):
                cell = ws_t20a[f'{col}{row}']
                cell.number_format = '#,##0' if col == 'D' else '0.0%'
        ...
        ws_t20c.append([])
        ws_t20c.append(['#', 'Customer No', 'Customer Name', 'Amount excl. VAT', '% of Total'])
        for i, c in enumerate(custs[:20], 1):
            ws_t20c.append([i, c['kundnr'], c['kund'], c['summa'], c['summa']/total_cust*100 if total_cust > 0 else 0])
        
        # 4. Full Articles sheet
        ws_art = wb_out.create_sheet("Articles")
        ws_art.append(['Article No', 'Article Name', 'Amount excl. VAT'])
        for a in arts:
            ws_art.append([a['artikelnr'], a['artikelnamn'], a['summa']])
        
        # 5. Full Customers sheet
        ws_cust = wb_out.create_sheet("Customers")
        ws_cust.append(['Customer No', 'Customer Name', 'Amount excl. VAT'])
        for c in custs:
